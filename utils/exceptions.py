...
class PipelineException(Exception):
    ...
    def __init__(self, message: str, layer: str = "unknown",
                 source: str = "unknown", context: dict | None = None):
        self.layer   = layer
        self.source  = source
...
class DataContractError(PipelineException):
    ...
    def __init__(self, source: str, expected_fields: list[str],
                 received_fields: list[str], **kw):
        ...
        super().__init__(
            ...
            layer="ingestion", source=source, context=ctx,
        )
...
class EntityResolutionError(PipelineException):
    ...
    def __init__(self, entity_type: str, raw_value: Any, candidates: list | None = None, **kw):
        ctx = {"entity_type": entity_type, "raw_value": str(raw_value),
               "candidates": candidates or []}
        super().__init__(
            f"Cannot resolve {entity_type} for value '{raw_value}'",
            layer="silver", source="entity_resolver", context=ctx,
        )
...
class DQCheckError(PipelineException):
    ...
    def __init__(self, check_name: str, table: str, pass_rate: float,
                 threshold: float, severity: str = "HIGH", **kw):
        if severity not in self.SEVERITIES:
            severity = "HIGH"
        ctx = {
            "check_name": check_name, "table": table,
            "pass_rate": round(pass_rate, 6), "threshold": threshold,
            "severity": severity,
        }
        super().__init__(
            f"DQ check '{check_name}' on '{table}' failed: "
            f"pass_rate={pass_rate:.4f} < threshold={threshold} [{severity}]",
            layer="silver", source="dq_monitor", context=ctx,
        )
...
class TemporalAlignmentError(PipelineException):
    ...
    def __init__(self, player_id: str, event_type: str,
                 event_ts: Any, anchor_ts: Any, **kw):
        ctx = {"player_id": player_id, "event_type": event_type,
               "event_ts": str(event_ts), "anchor_ts": str(anchor_ts)}
        super().__init__(
            f"Temporal misalignment for player {player_id}: "
            f"{event_type} at {event_ts} vs anchor {anchor_ts}",
            layer="enriched", source="temporal_aligner", context=ctx,
        )
...
class FeatureComputationError(PipelineException):
    ...
    def __init__(self, feature_name: str, player_id: str,
                 anchor_ts: Any, reason: str, **kw):
        ctx = {"feature": feature_name, "player_id": player_id,
               "anchor_ts": str(anchor_ts), "reason": reason}
        super().__init__(
            f"Feature '{feature_name}' computation failed for "
            f"player {player_id} at {anchor_ts}: {reason}",
            layer="gold", source="feature_engine", context=ctx,
        )
...
class DataLeakageError(PipelineException):
    ...
    def __init__(self, feature_name: str, anchor_ts: Any,
                 leaked_ts: Any, player_id: str | None = None, **kw):
        ctx = {"feature": feature_name, "anchor_ts": str(anchor_ts),
               "leaked_ts": str(leaked_ts), "player_id": player_id}
        super().__init__(
            f"DATA LEAKAGE DETECTED: feature '{feature_name}' includes data "
            f"at {leaked_ts} which is after anchor {anchor_ts}",
            layer="gold", source="leakage_guard", context=ctx,
        )
...
class FeatureStoreError(PipelineException):
    ...
    def __init__(self, operation: str, feature_view: str, reason: str, **kw):
        ...
        super().__init__(
            ...
            layer="feature_store", source="feature_store_client", context=ctx,
        )
...
class OODError(PipelineException):
    ...
    def __init__(self, player_id: str, reason: str,
                 distance_metric: float | None = None, **kw):
        ctx = {"player_id": player_id, "reason": reason,
               "distance_metric": distance_metric}
        super().__init__(
            f"OOD input detected for player {player_id}: {reason}",
            layer="ml", source="ood_detector", context=ctx,
        )
        self.ood_flag = True
...
if __name__ == "__main__":
    exceptions = [
        DataContractError("transfermarkt", ["player_id","injury_type","date_start"],
    ...
    for e in exceptions:
        print(f"  [{type(e).__name__}]  blocking={is_blocking(e) if isinstance(e, PipelineException) else 'N/A'}")
        print(f"    {e}")
