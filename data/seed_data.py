...
def get_injuries_df() -> pd.DataFrame:
    ...
    df["is_muscular"] = (
        df["injury_type"]
        .str.lower()
        .str.contains("muscle|hamstring|adductor|calf|thigh|strain", na=False)
        .astype(int)
    )
                ...
                rows.append(
                    {
                        "player_name": p,
                        "match_date": d,
                        "minutes_played": np.random.choice(
                            [90, 90, 90, 75, 60, 45], p=[0.4, 0.2, 0.1, 0.15, 0.1, 0.05]
                        ),
                        "venue": np.random.choice(["Bernabéu", "Away"], p=[0.5, 0.5]),
                        "competition": np.random.choice(
                            ["LaLiga", "UCL", "Copa"], p=[0.65, 0.25, 0.10]
                        ),
                    }
                )
...
def get_weather_df() -> pd.DataFrame:
    ...
    temp_diff = np.abs(np.random.normal(8, 3, len(dates))).clip(1, 15).round(1)
    temp_max_c = (temp_min_c + temp_diff).clip(-2, 42).round(1)
    return pd.DataFrame(
        {
            "date": dates,
